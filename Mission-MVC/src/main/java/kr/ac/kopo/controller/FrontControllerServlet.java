...
public class FrontControllerServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			switch(uri) {
			case "/board/list.do" :
			//이거 언제 다쓰나.. 전체 게시글 조회를 대신할 수 있는 class를 하나 만들 계획임. 그것의 이름은 BoardListController임.  
			//System.out.println("전체 게시글 조회서비스...");
				control = new BoardListController();
				callPage = control.handleRequest(request, response);
				//이건 이제 controller에 있는 interface에 있는 method임 묵시적 형변환을 사용해보자~! 
				/* BoardListController list = new BoardWriteFormController();
				 * callPage = list.handleRequest(request,response); 
				 * 
				 */
			//exception 처리를 안해서 에러남. try로 묶자. -> 우리는 지금 IOException이랑 ServletException만 처리중임. -> 그래서 try로 묶어주도록 해야 함. 
				break;
